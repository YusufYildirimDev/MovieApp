...
extension MovieDetailViewController: MovieDetailViewControllerProtocol {
    func showAlert() {
        let alertDialog = UIAlertController(title: "Uyarı!!!", message:"Favorilerden çıkartmak istediğinine eminmisin ?", preferredStyle: .alert)
        alertDialog.addAction(UIAlertAction(title: "Hayır", style: .cancel))
        alertDialog.addAction(UIAlertAction(title: "Evet", style: .default, handler: { action in
    ...
    func setFavoriteStatus(_ status: Bool) {
        favoriteImageView.image = status ? UIImage(systemName: "star.fill") : UIImage(systemName: "star")
    }
